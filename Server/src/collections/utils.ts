...
export async function refreshRecordingNames(
  ...
  const results =
    (await recordings.aggregate<IAggregateFullName>(query).toArray()) || [];

  for (const recording of results) {
    await recordings.findOneAndUpdate(
      { _id: recording._id },
      { $set: { fullName: recording.fullName } }
    );
  }
