...
export const Root: React.FC = observer(() => {
    ...
    const createContainer = pathname === `${routes.container}/NEW`
    const createRecording = pathname === `${routes.recording}/NEW`
    const createSeries = pathname === `${routes.series}/NEW`
    const createGenre = pathname === `${routes.genre}/NEW`
    const createLanguage = pathname === `${routes.language}/NEW`
    const create = createContainer || createRecording || createSeries || createGenre || createLanguage
    const container = pathname.startsWith(routes.container) && !create
    const genre = pathname.startsWith(routes.genre) && !create
    const language = pathname.startsWith(routes.language) && !create
    const recording = (pathname.startsWith(routes.recording) || pathname === '/') && !create
    const series = pathname.startsWith(routes.series) && !create

    const defaultTheme = (): void => rootStore.setTheme('default')

    const alternateTheme1 = (): void => rootStore.setTheme('alternate.1')

    const alternateTheme2 = (): void => rootStore.setTheme('alternate.2')

    const onClearErrors = (): void => rootStore.clearErrors()

    return (
        <div className='movie-db-root'>
            <Dimmer page active={isBusy || errors.length > 0}>
                <Loader active={isBusy} />
                <Message negative hidden={errors.length < 1} onDismiss={onClearErrors}>
            ...
            <Menu borderless>
                ...
                <Menu.Item>
                    <Dropdown text={tmui.title}>
                        <Dropdown.Menu>
                            <Dropdown.Item active={theme === 'default'} onClick={defaultTheme}>
                                {tmui.default}
                            </Dropdown.Item>
                            <Dropdown.Item active={theme === 'alternate.1'} onClick={alternateTheme1}>
                                {tmui.theme1}
                            </Dropdown.Item>
                            <Dropdown.Item active={theme === 'alternate.2'} onClick={alternateTheme2}>
                                {tmui.theme2}
                            </Dropdown.Item>
                        </Dropdown.Menu>
                    </Dropdown>
