...
describe('Pagination Component', () => {
  ...
  const mockCall = (
    params: Partial<RecordingsService>,
    result: Partial<IRecordingQueryResult>,
  ): void => {
  ...
  it('no data should be empty', () => {
    ...
    expect((selection[0].firstChild as HTMLElement).getAttribute('class')).toBe(
      'ui icon angle double left',
    );

    expect(selection[1].nodeName).toBe('A');
    expect((selection[1].firstChild as HTMLElement).getAttribute('class')).toBe(
      'ui icon angle left',
    );
    ...
    expect(selection[3].nodeName).toBe('A');
    expect((selection[3].firstChild as HTMLElement).getAttribute('class')).toBe(
      'ui icon angle right',
    );

    expect(selection[4].nodeName).toBe('A');
    expect((selection[4].firstChild as HTMLElement).getAttribute('class')).toBe(
      'ui icon angle double right',
    );
  });
